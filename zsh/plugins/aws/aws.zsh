...
function aws_prompt_info() {
  # use $AWS_VAULT if set, otherwise display $AWS_PROFILE
  local profile=${AWS_VAULT:-$AWS_PROFILE}

  # if neither $AWS_PROFILE nor $AWS_VAULT are set, do nothing
  [[ -n "$profile" ]] || return

  echo "${ZSH_THEME_AWS_PREFIX:=<aws:}${profile/%%}${ZSH_THEME_AWS_SUFFIX:=>}"
}
